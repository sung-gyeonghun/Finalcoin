        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and current_price < predicted_close_price:
                ...
                if krw > 5000:
                    ...
                    buy = current_price
                    buy2 = upbit.get_balance("KRW-ETH")
                    post_message(myToken,"#coin", "buy : " +str(buy)+"/"+str(buy2))
        else:
            eth = get_balance("ETH")
            if eth > 0:
                sell = current_price
                sell2 = upbit.get_balance("KRW-ETH")
                upbit.sell_market_order("KRW-ETH", eth)
                post_message(myToken,"#coin", "sell : " +str(sell)+"/"+str(sell2))
        time.sleep(1)
